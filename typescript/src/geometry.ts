...
export function convertToThreeGeometry(data: BufferGeometryData): THREE.BufferGeometry {
  ...
  geometry.setAttribute(
    ...
    new THREE.Float32BufferAttribute(
      data.data.attributes.position.array,
      data.data.attributes.position.itemSize,
    ),
  );
  geometry.setAttribute(
    "normal",
    new THREE.Float32BufferAttribute(
      data.data.attributes.normal.array,
      data.data.attributes.normal.itemSize,
    ),
  );
  ...
  if (data.data.attributes.uv) {
    geometry.setAttribute(
      "uv",
      new THREE.Float32BufferAttribute(
        data.data.attributes.uv.array,
        data.data.attributes.uv.itemSize,
      ),
    );
  } else {
...
export function createMeshFromGeometryData(
  data: BufferGeometryData,
  material: THREE.Material = new THREE.MeshStandardMaterial({ color: 0xcccccc }),
): THREE.Mesh {
