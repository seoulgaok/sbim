...
class Massing(BaseModel):
    ...
    orthogonal_north_cut: bool = Field(
        default=False,
        description=(
            "정북일조 한계선이 매스 축과 10° 이내로 비스듬해 남기는 쐐기 북벽을 "
            "직각(90°)으로 잘라낸다. 층당 면적이 줄 수 있다(연남동 365-24 4F: −9%). "
            "False=사선 보존(면적 우선, 기본). 필지별 선택 — 자동 기본값으로 켜면 "
            "타 필지 세대·주차가 회귀해 레버로 둔다."
        ),
    )
